...
class Trainer(object):
    ...
    def __init__(self, runtime_args: Namespace):
        ...
        timepath = str(time.time())
        exp_path = f'{exp_path}/{dataset}/{num_parts}part/{model_name}/{timepath}'
        # set up logger
        self.logger = setup_logger(f'logs/'+str(time.time())+f'.trainer.log', log_level, with_file=True)
        # set up communicator
    ...
    *************************************************
    ...
    def _set_assigner(self):
        ...
        self.assigner = assigner(data_config['num_feats'], model_config['hidden_dim'], model_config['num_layers'], assignment_config['profile_data_length'], runtime_config['assign_scheme'], assignment_config['assign_bits'], engine.ctx.scores, assignment_config['group_size'], assignment_config['coe_lambda'], assignment_config['assign_cycle'], warmup = 1, exp_path = self.exp_path)
