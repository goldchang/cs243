...
class Assigner(object):
    ...
    '''
    def __init__(self, feat_dim: int, hidden_dim: int, num_layers: int, num_data: int, scheme: str, uniform_assign_bits: int, scores: Dict[int, Tuple[Tensor, Tensor]], group_size: int, coe_lambda: float, assign_cycle: int = None, warmup: int = 1, exp_path: str = ""):
        # basic parameters
        ...
        self.layer_assignments: Dict[str, Dict[str, Tensor]] = {}
        self.assignment_count = 0
        self.layer_solve_handles = Queue()
    ...
    *************************************************
    ...
    def aggregate_params_get_solution(self, var_matrix: Dict[str, Dict[str, np.ndarray]], comm_matrix: Dict[str, Dict[str, np.ndarray]], coe_lambda: float) -> Dict[str, Dict[int, Tensor]]:
        ...
        if rank != 0:
            ...
            if os.getenv("OVERLAP", "0") == "1" and self.assignment_count != 0:
                allset = True
                for _, handle in list(self.layer_solve_handles.queue):
                    if not handle.ready():
                        allset = False
                        print("Found handle not ready")
                        break
                if allset == True:
                    solving_times = []
                    print("Allset is true")
                    while not self.layer_solve_handles.empty():
                        print("Queue not empty")
                        layer_key, handle = self.layer_solve_handles.get()
                        solving_time, assignment = handle.get()
                        if solving_time == -1:
                            print("Early Stopping")
                            continue
                        else:
                            self.layer_assignments[layer_key] = assignment
                            logger.info(f'layer {layer_key} solving time: {solving_time:.4f}s')
                            solving_times.append(f'layer {layer_key} solving time: {solving_time:.4f}s')
                    with open(self.exp_path + "/lp.txt", "a") as file:
                        for solving_time in solving_times:
                            file.write(solving_time + "\n")
                    self.assignment_count+=1
                    for layer_key in layer_keys:
                        print("Generating new problems")
                        handle = self.solving_pool.apply_async(self.get_solution, args=(all_layer_var[layer_key], all_layer_comm[layer_key], all_layer_model, coe_lambda, world_size, 'nadir_utopia', layer_key, False))
                        self.layer_solve_handles.put((layer_key, handle))

            if os.getenv("OVERLAP", "0") == "0" or (os.getenv("OVERLAP", "0") == "1" and self.assignment_count == 0):
                for layer_key in layer_keys:
                    print(layer_key)
                    if self.assignment_count == 0:
                        print("No Early Stopping")
                    handle = self.solving_pool.apply_async(self.get_solution, args=(all_layer_var[layer_key], all_layer_comm[layer_key], all_layer_model, coe_lambda, world_size, 'nadir_utopia', layer_key, self.assignment_count == 0))
                    self.layer_solve_handles.put((layer_key, handle))
            # wait for solution
            solving_times = []
            if self.assignment_count == 0 or os.getenv("SKIP", "0") == "0" or os.getenv("OVERLAP", "0") == "0":
                while not self.layer_solve_handles.empty():
                    print("Assignment count 0 queue not empty")
                    layer_key, handle = self.layer_solve_handles.get()
                    solving_time, assignment = handle.get()
                    if solving_time == -1:
                        print("Early Stopping")
                ...
                self.assignment_count+=1
            # scatter assignments
            assignments_list = rearrange_scatter_assignments(self.layer_assignments, world_size)
            comm.barrier()
    ...
    *************************************************
    ...
    def get_solution(self, var_matrix: Dict[str, Dict[str, np.ndarray]], comm_matrix: Dict[str, Dict[str, np.ndarray]], cost_model: Dict[str, np.ndarray], coe_lambda: float, world_size: int, normal_mode: str = 'nadir_utopia', layer_key: str = '', first_time: bool = False):
        #real_func()

        pickle.dump(self.bits_set, open("" + layer_key + "_bits_set.dat", "wb"))
        pickle.dump(var_matrix, open("" + layer_key + "_var_matrix.dat", "wb"))
        pickle.dump(comm_matrix, open("" + layer_key + "_comm_matrix.dat", "wb"))
        pickle.dump(cost_model, open("" + layer_key + "_cost_model.dat", "wb"))
        pickle.dump(coe_lambda, open("" + layer_key + "_coe_lambda.dat", "wb"))
        pickle.dump(world_size, open("" + layer_key + "_world_size.dat", "wb"))
        pickle.dump(normal_mode, open("" + layer_key + "_normal_mode.dat", "wb"))
        #pickle.dump(layer_key, open(layer_key + "_layer_key.dat", "wb"))
        pickle.dump(first_time, open("" + layer_key, "wb"))
        ...
        #for event in inotify.read():
            #print(event.name)
            #if event.name == layer_key + "_done":

        while True:
            for file in os.listdir(os.fsencode("")):
                filename = os.fsdecode(file)
                if filename == layer_key:
                    os.remove("" + layer_key)
                    solving_time = pickle.load(open("" + layer_key + "_solving_time.dat", "rb"))
                    bits_assignment = pickle.load(open("" + layer_key + "_bits_assignment.dat", "rb"))
        ...
        start = time.time()
        current_solver = os.getenv("SOLVER", "GUROBI")
        ...
        if current_solver in available_solvers and current_solver == "GUROBI":
            opt_model.solve(plp.GUROBI(msg=False))
        elif current_solver == "CPLEX_PY":
            opt_model.solve(plp.CPLEX_PY(msg=True))
        elif current_solver in available_solvers and current_solver == "GLPK_CMD":
            opt_model.writeMPS("test_model.mps")
            var, opt_model = plp.LpProblem.fromMPS("test_model.mps")
            opt_model.solve(plp.GLPK_CMD(msg=True)) # for glp we'll just do this manually
        elif current_solver == "HiGHS_CMD":
            opt_model.solve(plp.HiGHS_CMD(msg=True))
        else:
            opt_model.solve(plp.PULP_CBC_CMD(msg=True))
